...
if ($num_rows > 0) {
    while ($row = $resultado->fetch_assoc()) {
        ...
        $output['data'] .= '<tr class="py-3 px-6 border border-4 border-blue-400">';
        $output['data'] .= '<td class="py-3 px-6 border border-4 border-blue-400">' . $contador. '</td>';
        $output['data'] .= '<td class="py-3 px-6 border border-4 border-blue-400">' . $nombre. '</td>';
        $output['data'] .= '<td class="py-3 px-6 border border-4 border-blue-400">' . $descripcion . '</td>';
        $output['data'] .= '<td class="py-3 px-6 border border-4 border-blue-400">' . $ruta. '</td>';
        $output['data'] .= '<td class="py-3 px-6 border border-4 border-blue-400">' . $fecha. '</td>';
        $output['data'] .= '<td class="py-3 px-6 border border-4 border-blue-400">' . $categoria1 . '</td>';
        $output['data'] .= '<td class="py-3 px-6 border border-4 border-blue-400"><a class="focus:outline-none text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800" target="_blank"  href='.$enlace.'>Visualizar</a></td>';
        $output['data'] .= '</tr>';
    }
} else {
    $output['data'] .= '<tr>';
